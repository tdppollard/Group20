...
cor1990 %>%
  ...
  labs(y = "Correlation with log GDP") + geom_errorbar(aes(ymax = CI1990[2,], ymin = CI1990[1,]), width = 0.5)
```
...
cor2015 %>%
  rownames_to_column("Group") %>%
  ggplot(aes(Group, Correlation)) + geom_col() + theme(axis.text.x = element_text(angle = -45)) +
  labs(y = "Correlation with log GDP")+ geom_errorbar(aes(ymax = CI2015[2,], ymin = CI2015[1,]), width = 0.5)
```
